...
def send_alert(env, data):
    ...
    if not token:
        current_app.logger.error(ERROR_TOKEN_NOT_SET)
        return
    if not secret:
        current_app.logger.error(ERROR_SECRET_NOT_SET)
        return
    ...
    for alert_name, alerts_group in alert_dict.items():
        ...
        # 检查发送结果
        if result['errcode'] != 0:
            current_app.logger.error('notify dingtalk error: %s' % result['errcode'])
